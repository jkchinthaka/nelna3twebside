  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="relative z-10">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
          <div className="flex items-center gap-2">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-600 text-white font-bold text-xl shadow-lg shadow-brand-200">N</span>
            <div>
               <h1 className="font-heading text-xl font-bold tracking-tight text-slate-900">Nelna</h1>
               <p className="text-xs text-slate-500 font-medium tracking-wider uppercase">Product Finder</p>
            </div>
          </div>
          <div className="rounded-full bg-white px-4 py-1.5 text-xs font-bold text-brand-700 shadow-sm ring-1 ring-slate-100">
            MVP 1.0
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 pb-20 pt-8 sm:px-6">
        ...
        {error && (
          <div className="mx-auto mt-8 max-w-2xl rounded-xl border border-red-100 bg-red-50 p-4 text-center text-sm text-red-600 shadow-sm">
            {error}
          </div>
        )}
        ...
        {results && (
          <section className="mt-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
             <div className="flex items-baseline justify-between mb-6">
                <h2 className="font-heading text-2xl font-bold text-slate-900">Search Results</h2>
                <span className="text-sm text-slate-500">{stores.length} stores found found nearby</span>
             </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {results.products.map((product) => (
                      ...
                      >
                        ...
                        <p className="mt-1 font-heading font-bold text-slate-900">{product.name}</p>
                        <div className="mt-3 flex items-center gap-2">
                           <div className="h-1.5 flex-1 rounded-full bg-slate-100 overflow-hidden">
                              <div className="h-full bg-brand-500 rounded-full" style={{ width: `${product.score * 100}%` }} />
                           </div>
                           <span className="text-xs font-medium text-slate-500">{(product.score * 100).toFixed(0)}% Match</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-4">
                  {stores.length > 0 ? (
                     ...
                     <div className="text-center py-12 rounded-3xl border-2 border-dashed border-slate-200">
                        <p className="text-slate-400">No stores found with these items nearby.</p>
                     </div>
                  )}
                </div>
              </div>
        ...
        {!results && (
          <div className="mt-20 text-center">
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto opacity-40 grayscale hover:grayscale-0 transition-all duration-700">
                <div className="aspect-video client-logo bg-slate-100 rounded-xl" />
                <div className="aspect-video client-logo bg-slate-100 rounded-xl" />
                <div className="aspect-video client-logo bg-slate-100 rounded-xl" />
                <div className="aspect-video client-logo bg-slate-100 rounded-xl" />
             </div>

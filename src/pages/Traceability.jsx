  ...
  return (
    <div className="mx-auto max-w-5xl space-y-10 px-4 py-12">
      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-8">
        <label className="text-xs font-semibold uppercase tracking-widest text-slate-500">Batch ID</label>
        <input
          value={batchId}
        ...
        />
        <p className="mt-2 text-xs text-slate-500">This demo uses internal sample batch data until live system integration.</p>
      </div>

      {batchId && !result && (
        <div className="rounded-3xl border border-slate-100 bg-slate-50 p-8 text-center">
          <p className="text-sm font-semibold text-slate-600">No batch found.</p>
          <p className="mt-2 text-xs text-slate-500">Please check the batch ID printed on the pack label.</p>
        </div>
      )}

      {result && (
        <div className="rounded-3xl border border-slate-100 bg-white p-8">
          <h3 className="text-lg font-semibold text-slate-900">Batch {result.batchId}</h3>
